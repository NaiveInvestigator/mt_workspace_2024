...
status_led = rospy.Publisher("/rgb_state", String, queue_size=10)
rate = rospy.Rate(5)
...
def trajectory_callback(msg):
    data_strings = msg.data.split() # lat, lon, post_state
    lat = data_strings[0]
    lon = data_strings[1]
    post_state = data_strings[2]
    mission(target_lat=lat, target_lon=lon, post=post_state)


trajectory_inp = rospy.Subscriber("/trajectory_input", String, trajectory_callback)
...
def mission(target_lat, target_lon, post=False, distance_lim=1.0):
    ...
    if post == True:
        ...
        while not post_state:
            if aruco_state == 0:
                ...
                if x > ((img_res[0]/2) + 50):
                    ...
                    if distance > distance_lim:    
                        ...
                        if distance == 0.0:
                            ...
                            rover.publish("-")
                            print(distance)
                            status_stuff(f"Within set distance, {distance_lim}")
                            post_state = True
                ...
                #rover.publish("-")
                rate.sleep()
    status_stuff("Reached!")

if __name__ == "__main__":
    rover.publish("7")
    #coords = [[23.7752259, 90.4090382, 1]] # lat, lon, post_state
             # [90.409118, 23.774902, 1], # lon, lat, post_state
    ...
    #for coord in coords:
    #    mission(target_lat=coord[0], target_lon=coord[1], post=bool(coord[2]))
