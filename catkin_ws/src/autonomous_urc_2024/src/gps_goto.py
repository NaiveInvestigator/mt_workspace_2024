...
class Navigation():
    ...
    def rotate_180(self, target_yaw, yaw_threshold = 10):
        global yaw

        while aruco_state or object_state:
            print("Paused")
            pass

        callbacks.status_stuff("Looking for tag/object")
        target_yaw = yaw + 180
    ...
    def moveTo(self, coords, yaw_threshold = 10, avoid_obstacles = False, perform_search = False):  # coords = {"longitude": 0, "latitude": 0}
        ...
        og_lat, og_lon = lat, lon
        if perform_search:
            callbacks.status_stuff("Looking for tag/object")

            self.rotate_180(yaw + 180)
            self.rotate_180(yaw + 180)

            for dist in [5, 8]:
                self.hex_search(og_lat, og_lon, dist)
                self.rotate_180(yaw + 180)
                self.rotate_180(yaw + 180)
...
if __name__ == "__main__":
    ...
    # navigation.hex_search(lat, lon, 5)