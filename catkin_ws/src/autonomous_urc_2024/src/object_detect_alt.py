...
def rgb_image_callback(data):
    ...
    #         cv2.putText(img, "Mallet", top_left, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    bottle_top_left = None
    bottle_bottom_right = None

    mallet_top_left = None
    mallet_bottom_right = None

    mallet_max_conf = float("-inf")
    bottle_max_conf = float("-inf")

    for obj in result.boxes.data:
        obj = obj.tolist()
        if obj[4] >= conf_threshold: #and obj[5] == float(id_name_mapping[trg_label]):
            if obj[5] == 0:
                if obj[4] > bottle_max_conf:
                    bottle_x_centre = int((obj[0] + obj[2]) / 2)
                    bottle_y_centre = int((obj[1] + obj[3]) / 2)
                    bottle_top_left = (int(obj[0]), int(obj[1]))
                    bottle_bottom_right = (int(obj[2]), int(obj[3]))
                    bottle_max_conf = obj[4]
            else:
    ...
    if bottle_max_conf >= conf_threshold:
        cv2.circle(img, (bottle_x_centre, bottle_y_centre), 5, (255, 0, 0), -1)
        cv2.rectangle(img, (bottle_top_left[0], bottle_top_left[1]), (bottle_bottom_right[0], bottle_bottom_right[1]), (0, 0, 255), 3)
        cv2.putText(img, "Bottle", bottle_top_left, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)

        object_state = "1"
        x, y = bottle_x_centre, bottle_y_centre
        distance = depth_frame[y, x]
        data_string = object_state + " " + str(distance) + " " + str(x) + " " + str(y) 
        print(f"Center: {x}, {y}")
        print(f"Distance: {distance}")

    if mallet_max_conf >= conf_threshold:
